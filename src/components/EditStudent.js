...
function EditStudent()
{
    ...
    const {id} = useParams();
    useEffect(()=>{
        Axios.get("https://crud-deployment1-6r39.onrender.com/students/update-student/" + id)
        .then((res)=>{
            if(res.status === 200){
    ...
    const handleSubmit = () =>{
        ...
        Axios.put("https://crud-deployment1-6r39.onrender.com/students/update-student/"+ id, data)
        .then((res)=>{
            if(res.status === 200) {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <StudentForm getState={getState}
                        ...
                        rollNoValue = {initialValue.rollNo}>Update Student</StudentForm>
        </form>
    )
}
